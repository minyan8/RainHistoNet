...
from basicsr.models.archs.histoformer_arch import TransformerBlock as hist
factor = 2
class UHDM(nn.Module):
    def __init__(self,
        ...
        # self.igab = IGAB(dim=4 * en_feature_num, dim_head=en_feature_num, heads=1)
...
class Encoder_Level(nn.Module):
    def __init__(self, feature_num, inter_num, level, sam_number):
        ...
        if self.level == 3:
            ...
            self.hist = hist(dim=feature_num, num_heads=1,  ffn_expansion_factor=2.67, bias=False, LayerNorm_type='WithBias')

    def forward(self, x):
        out_feature = self.rdb(x)
        # out_feature = self.norm(out_feature)
        # out_feature = self.ig_msa(out_feature) + x
        # out_feature = self.norm1(out_feature)
        # out_feature = out_feature.permute(0, 2, 3, 1)  
        # out_feature = self.igab(out_feature) # sysu+igab before sam
        # out_feature = out_feature.permute(0, 3, 1, 2)
        for sam_block in self.sam_blocks:
            out_feature = sam_block(out_feature)
        if self.level == 3:
            # out_feature = self.igab(out_feature)
            out_feature = self.hist(out_feature)
        if self.level < 3:
...
class Decoder_Level(nn.Module):
    def __init__(self, feature_num, inter_num, sam_number):
        ...
        # self.norm = self.norm = nn.GroupNorm(num_groups=1, num_channels=feature_num)
        # self.ig_msa = IG_MSA(dim=feature_num, dim_head=feature_num, heads=1)
        # self.norm1 = self.norm = nn.GroupNorm(num_groups=1, num_channels=feature_num)
        # self.igab = IGAB(dim=feature_num, dim_head=feature_num, heads=1)
        self.sam_blocks = nn.ModuleList()
        for _ in range(sam_number):
            sam_block = SAM(in_channel=feature_num, d_list=(1, 2, 3, 2, 1), inter_num=inter_num)
        ...
        self.conv1 = conv(in_channel=feature_num, out_channel=64, kernel_size=3, padding=1)

    def forward(self, x, feat=True):
        # x_copy = x
        x = self.rdb(x)
        # x = self.norm(x)
        # x = self.ig_msa(x) + x_copy
        # x = self.norm1(x)
        # x = x.permute(0, 2, 3, 1)      
        # x = self.igab(x) # sysu+igab before sam
        # x = x.permute(0, 3, 1, 2)
        for sam_block in self.sam_blocks:
            x = sam_block(x)
        # x = self.conv1(x)
